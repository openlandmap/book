...
To enumerate all available collections in the OpenLandMap catalog, we can scrutinize the links entry and filter only `"child" relations:
...
Now, let's list its available items by filtering links with the `rel == "item"` filter expression:

```{r}
links(glc, rel == "item")
```

To be able to filter items based on spatial and temporal attributes, we need to read them:
