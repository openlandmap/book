...
Using Altamira municipality's boundaries in Brazil's Pará state, we'll extract zonal statistics over time. The municipality data comes from the `gadm` data in the `geodata` package, stored as a `terra` `SpatVector` object. Due to its proximity to areas of high biodiversity and its history of land-use change, Altamira has been in focus of conservation debate. The agricultural expansion and access to major transportation routes have contributed to its prominence in the ongoing deforestation in the Amazon region.
...
Now we get the URLs for the monthly `"fapar_essd.lstm_p95_250m_s"` asset. This asset represents the 95th percentile of FAPAR values observed within each month. This corresponds to high FAPAR values. Now, our focus shifts to computing the 5th percentile of FAPAR values within the Altamira territory. This approach allows us to explore the lower 5% of the FAPAR values observed in Altamira area.

```{r}
quantiles <- seq(0.05, 0.15, 0.01)
quantiles_name <- gsub("0\\.", "p", quantiles)
fapar_q <- fapar_items %>%
...
ggplot(fapar, aes(x = datetime, y = fapar_p95)) +
  geom_line()
```
A decreasing 5th percentile of FAPAR p95 values suggests the increasing of areas with lower vegetation than the threshold set by the p95 value.
This could indicate regions within Altamira that experience relatively lower vegetation density or health compared to the broader region represented by the p95 values.
