...
For overlaying multiple new points with COGs, we can leverage the `rstac` function `assets_url()` to retrieve the URLs of all COG files. These URLs can then be used to read the COGs:
...
In this code, the `append_gdalvsi=TRUE` parameter is set to inform the function to add the string `"/vsicurl/"` to each URL. This step is necessary for GDAL to open the files properly.
...
Let's organize the extracted values as a tibble column. We can use a `tibble` derived from `glc_items` using the `items_as_tibble` function:
...
In this section, we're obtaining municipal boundaries for Brazilian Pará state using `gadm` data extracted from `geodata` package. The boundaries are stored as `SpatVector` object. In this example, we use the boundary of the municipality of Altamira in Pará state.
...
x <- 0:21
y <- unlist(
  exact_extract(fapar, sf::st_as_sf(altamira), 'quantile', quantiles = 0.05), 
  use.names = FALSE
)
