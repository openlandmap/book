...
In this section, we're using municipal boundaries for Brazilian Pará state using `gadm` data extracted from `geodata` package. The boundaries are stored as `SpatVector` object. In this example, we use the boundary of the municipality of Altamira in Pará state.
